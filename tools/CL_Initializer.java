 ...
 * @version $Id: CL_Initializer.java 1160 2012-12-12 17:43:16Z ucacpco $
 * 
 */
public class CL_Initializer {
	...
	public static void initSphFuncDataSource() {
		if (inputModel == null) {
			...
			data = new VoxelOrderDataSource(inputFile, ONETENVALSPERVOXEL,
					inputDataType);
		} else if (inputModel.equals("ballstick")) {
			data = new VoxelOrderDataSource(inputFile, BALLSTICKVALSPERVOXEL,
					inputDataType);
			...
			data = new VoxelOrderDataSource(inputFile, RBF_Sum.numPoints() + 2,
					inputDataType);
		} else if (inputModel.equals("maxent")) {
			data = new VoxelOrderDataSource(inputFile, MaxEntProfile
					.numLambdas() + 2, inputDataType);
		} else {
			RuntimeException e = new RuntimeException(
	...
	public static void initTensorDataSource() {
		...
		if (inputModel.equals("dt")) {
			...
			data = new VoxelOrderDataSource(inputFile, TWOTENVALSPERVOXEL,
					inputDataType);
		} else if (inputModel.equals("threetensor")) {
			data = new VoxelOrderDataSource(inputFile, THREETENVALSPERVOXEL,
					inputDataType);
		} else if (inputModel.equals("multitensor")) {
			data = new VoxelOrderDataSource(inputFile,
					3 + 7 * maxTensorComponents, inputDataType);
		} else {
			throw new LoggedException(
		...
		if (bgMaskFile != null) {
			...
			if (ImageHeader.imageExists(bgMaskFile)) {
				...
				try {
					...
					throw new LoggedException("Cannot read mask header "
							+ bgMaskFile);
				}
	...
	public static void initDataSynthesizer() {
		...
		if (p != null) {
			...
			if (bootstrap > 0 && wildBootstrapModel != null) {
				...
				DataSource input = getDataSynthesizer(p, imPars, SNR, 1, seed,
						-1);

				if (wildBootstrapModel.equals("dt")) {
					data = new WildBS_DT_DataSynth(input, imPars, numVoxels,
							seed);
				} else {
					throw new LoggedException(
							"Wild bootstrap is not supported for model: "
									+ wildBootstrapModel);
				}

			} else {
				data = getDataSynthesizer(p, imPars, SNR, numVoxels, seed,
						bootstrap);
			}
			...
			if (compartmentModel) {
				if (!modelfitting) {
					// compartment model has already been initialised if we're
					// here, so do nothing
					data = new CompartmentModel(compartmentNames, compParams,
							imPars);
				}
			}
			if (brownianSimulation) {
				...
				if (SimulationParams.trajectories) {
					...
					try {
						SimulableScheme simScheme = (SimulableScheme) imPars;
					} catch (ClassCastException cce) {
						throw new LoggedException(
								"specified schemefile is not compatible with simulation. dynamics require all scan params to be specified.");
					}
				...
				stepParams = StepGeneratorFactory.getStepParamsArray(
						SimulationParams.sim_stepType, simParams);
				...
				if (bootstrap > 0) {
					...
					DiffusionSimulation sim = new DiffusionSimulation(
							simParams, (SimulableScheme) imPars);
					...
					if (wildBootstrapModel != null) {
						if (wildBootstrapModel.equals("dt")) {
							data = new WildBS_DT_DataSynth(data, imPars,
									numVoxels, seed);
						} else {
							throw new LoggedException(
									"Wild bootstrap is not supported for model: "
					...
					data = new BootstrapDataSynthFromSimulationInput(sim,
							bootstrap, imPars.numMeasurements(), numVoxels,
							seed);
				} else {
					if (SimulationParams.trajectories) {
			...
			/*
			 * if(compartmentModel){ if(!modelfitting){ // compartment model has
			 * already been initialised if we're here, so do nothing data= new
			 * CompartmentModel(compartmentNames, compParams, imPars); } }
			 */
			else if (bootstrap > 0) {
				...
				if (wildBootstrapModel != null) {
					...
					data = new VoxelOrderDW_DataSource(inputFile,
							inputDataType, imPars);

					if (wildBootstrapModel.equals("dt")) {
						data = new WildBS_DT_DataSynth(data, imPars, numVoxels,
								seed);
					} else {
						throw new LoggedException(
								"Wild bootstrap is not supported for model: "
					...
					data = new BootstrapDataSynthesizer(bsDataFiles, imPars
							.numMeasurements(), numVoxels, inputDataType, seed);
				}
			...
			if (bootstrap > 0) {
				...
				if (inputModel.equals("dt")) {
					input = new DataSynthFromDT_Input(inputFile, inputDataType,
							imPars, inputSNR, seed);
				} else if (inputModel.equals("twotensor")) {
					input = new DataSynthFromTwoTensorInput(inputFile,
							inputDataType, imPars, inputSNR, seed);
				} else if (inputModel.equals("threetensor")) {
					input = new DataSynthFromThreeTensorInput(inputFile,
							inputDataType, imPars, inputSNR, seed);
				} else if (inputModel.equals("multitensor")) {
					input = new DataSynthFromMultiTensorInput(inputFile,
							inputDataType, imPars, maxTensorComponents,
							inputSNR, seed);
				} else if (inputModel.equals("ballstick")) {
					input = new DataSynthFromBallStickInput(inputFile,
							inputDataType, imPars, inputSNR, seed);
				} else {
				...
				if (wildBootstrapModel != null) {
					if (wildBootstrapModel.equals("dt")) {
						data = new WildBS_DT_DataSynth(input, imPars,
								numVoxels, seed);
					} else {
						throw new LoggedException(
								"Wild bootstrap is not supported for model: "
				...
				if (inputModel.equals("dt")) {
					data = new DataSynthFromDT_Input(inputFile, inputDataType,
							imPars, SNR, seed);
				} else if (inputModel.equals("twotensor")) {
					data = new DataSynthFromTwoTensorInput(inputFile,
							inputDataType, imPars, SNR, seed);
				} else if (inputModel.equals("threetensor")) {
					data = new DataSynthFromThreeTensorInput(inputFile,
							inputDataType, imPars, SNR, seed);
				} else if (inputModel.equals("multitensor")) {
					data = new DataSynthFromMultiTensorInput(inputFile,
							inputDataType, imPars, maxTensorComponents, SNR,
							seed);
				} else if (inputModel.equals("ballstick")) {
					data = new DataSynthFromBallStickInput(inputFile,
							inputDataType, imPars, SNR, seed);
		...
		} // matches else (ie what we do when p == null and inputmodel !=
			// null)
	 ...
	 * 
	 * @return The <code>ModelPDF</code> object encapsulating the test
	 *         function.
	 */
	public static ModelPDF constructTestFunction(double[] testFunctionData) {
		...
		for (int i = 0; i < components; i++) {
			...
			if (rotationIndex > 0) {
				dts[i] = dts[i].transform(Rotations.randomRotMat(new Random(
						rotationIndex)));
			}
	 ...
	 *            <code>DataSynthesizer</code> object. If greater than zero,
	 *            it returns a <code>SyntheticBootstrapper</code> with
	 *            <code>bootstrap</code> repeats.
	 * 
	 * @return The source of data.
	 */
	public static DataSource getDataSynthesizer(ModelPDF p, DW_Scheme imPars,
		...
		if (bootstrap <= 0) {
			...
			data = new SyntheticBootstrapper(p, imPars, SNR, bootstrap,
					numVoxels, seed);
		}
