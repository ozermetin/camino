...
public class ParallelCylinderSubstrate extends CylinderSubstrate{
	...
	public ParallelCylinderSubstrate(SimulationParams simParams){
		...
		super(new double[]{2*SimulationParams.sim_cyl_R, 
		                   2*SimulationParams.sim_cyl_R, 
		                   2*SimulationParams.sim_cyl_R}, simParams, false);
		...
		if(SimulationParams.sim_cyl_R<2.0*SimulationParams.sim_cyl_r){
			logger.warning("Cylinder spacing on parallel cylinder lattice (R="
					+SimulationParams.sim_cyl_R+") is less than cylinder radius (r="
					+SimulationParams.sim_cyl_r+")");
		}
		

		this.R=SimulationParams.sim_cyl_R;
	        
		double r=SimulationParams.sim_cyl_r;
		...
		if(packing==SQUARE){
			...
			P[0]=0.0;
			P[1]=0.0;
			cylinder[counter++]= new BasicCylinder(P, r, p);
	...
	public ParallelCylinderSubstrate(CylType type, SimulationParams simParams){
		...
		double r2= SimulationParams.sim_cyl_r;
	...
	public static void main(String[] args){
		...
		boolean crosses= pcs.crossesMembrane(walker, offset, s, normal, d, false, 0.2, in, p);
		...
		substrate.crossesMembrane(walker, offset, s, normal, d, false, 0.2, in, p);
		System.err.println("crosses is "+crosses+" normal=("+normal[0]+","+normal[1]+","+normal[2]+") d="+d[0]);
