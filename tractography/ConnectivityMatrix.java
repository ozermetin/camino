...
public abstract class ConnectivityMatrix  {
    ...
    /**
     * Remap targets to a continuous series of integers. 
     *
	...
	for (int i = 0; i < xDataDim; i++) {
		...
		for (int k = 0; k < zDataDim; k++) {
			...
			if (index == null) {
			    throw new LoggedException("Target file contains unidentified label " + targetROIs[i][j][k]);
			}
