 ...
 * <dt>Purpose:
 * 
 * <dd>Fits the tensor and gamma distributed radii cylinders and sphere model using one run of a Levenburg Marquardt.
 * 
 * <dt>Description:
 * 
 ...
 * 
 */
public class TensorGDRCylindersSphereLM_Fitter extends CompartmentFitter {

	protected TensorCylinderLM_Fitter tcfitter;

	private final int numModelParams = 17;
	private final int numOptParams = 12;

	/**
	 * Default constructor required for derived classes.
	 */
	public TensorGDRCylindersSphereLM_Fitter() {
	}
	 ...
	 */
	public TensorGDRCylindersSphereLM_Fitter(DW_Scheme scheme) {
		...
		double[] initialParams = new double[17];
	...
	protected void makeCodec() {
		cod = new Codec() {
			...
			public double[] modelToOpt(double[] modelParams) {
				...
				optParams[9] = (Math.asin(Math.sqrt(modelParams[13]/ modelParams[12])));//diffprp constrained
				optParams[10] = modelParams[14];//alpha
				optParams[11] = (Math.acos(Math
						.sqrt((modelParams[16] - 1e-7) / 40e-6)));// Rglial constrained
			...
			public double[] optToModel(double[] optParams) {
				...
				modelParams[12] = Math.sin(optParams[8]) * Math.sin(optParams[8]);//diff perp constrained
				modelParams[12] = modelParams[12] * modelParams[6];//diff perp constrained
				modelParams[13] = Math.sin(optParams[9]) * Math.sin(optParams[9]);//diff perp constrained
				modelParams[13] = modelParams[13] * modelParams[12];//diff perp constrained
				
				modelParams[14] = optParams[10];//alpha 
				modelParams[15] = modelParams[6]; // diff astro
				modelParams[16] = 1e-7 + (Math.cos(optParams[11])
						* Math.cos(optParams[11]) * 40e-6);// Rglial constrained
	...
	protected double[] getStartPoint(double[] data) {
	    ...
	    modelParams[11] =phi;
	    modelParams[12] =diffperp1;
	    modelParams[13] =diffperp2;
	    modelParams[14] =alpha;
	    modelParams[15] = diff;
	    modelParams[16] = Rg;
		return modelParams;
	...
	public static void main(String[] args) {
		...
		TensorGDRCylindersSphereLM_Fitter inv = new TensorGDRCylindersSphereLM_Fitter(
