 ...
 * <dt>Purpose:
 * 
 * <dd>Fits the zeppelinstick and sphere model using one run of a
 * Levenburg Marquardt.
 * 
 * <dt>Description:
 * 
 * <dd> The diffusivity parameter is assumed the same in each compartment.
 * Parallel diffusivity, unweighted signal and the radius of the astrocylinders
 * are constrained positive by optimizing their square root. Volume fraction is
 * constrained to [0 1] by optimizing cos^{-1} sqrt(f). Perpendicular
 * diffusivity is constrained to be lower than the parallel diffusivity by
 * optimising sin^(-1)sqrt(dperp/dparal). The radius is constrained to 20
 * microns by optimizing cos^{-1} sqrt(R/20e-6).
 * 
 * </dl>
 * 
 ...
 * 
 */
public class ZeppelinStickSphereLM_Fitter extends CompartmentFitter {

	protected ZeppelinStickLM_Fitter zsfitter;
	protected ZeppelinStickAstrosticksLM_Fitter zsafitter;
	 ...
	 */
	public ZeppelinStickSphereLM_Fitter() {
	}
	 ...
	 */
	public ZeppelinStickSphereLM_Fitter(DW_Scheme scheme) {
		...
		cod = new Codec() {
			...
			public double[] modelToOpt(double[] modelParams) {
				...
				optParams[7] = (Math.acos(Math
						.sqrt((modelParams[12] - 1e-7) / 40e-6)));// R
			...
			public double[] optToModel(double[] optParams) {
				...
				modelParams[12] = 1e-7 + (Math.cos(optParams[7])
						* Math.cos(optParams[7]) * 40e-6);// R constraining
															// the radius
	...
	/**
	 * Estimates a starting set of parameters from the zeppelinstick.
	 * 
	 * @param data
	...
	protected double[] getStartPoint(double[] data) {
		...
		try {
			zsaparams = MultiRunMinimizer.getBestSolution(zsafitter.fit(data));

		} catch (MinimizerException e) {

			zsaparams = zsafitter.getStartPoint(data);
			//e.printStackTrace();
		}
	...
	public static void main(String[] args) {
		...
		ZeppelinStickSphereLM_Fitter inv = new ZeppelinStickSphereLM_Fitter(
