...
rm -f ${SCRIPTDIR}/crossing/results ${SCRIPTDIR}/torus/results

mkdir ${SCRIPTDIR}/crossing/results ${SCRIPTDIR}/torus/results
...
cpstats -operation ${operation} -inputroot torus/results/torus_single_roi_cp_1_ -outputfile torus/results/torus_single_roi_cp_1_${operation} -gzip
done

# Compare results
echo "TRACKING TEST $TEST_COUNTER"; TEST_COUNTER=$(($TEST_COUNTER + 1)) # $1 + 3
for operation in max min median mean; do
    imagessd torus/results/torus_single_roi_cp_1_${operation}.hdr torus/expectedResults/torus_single_roi_cp_1_${operation}.hdr
done
...
cpstats -operation max -inputroot torus/results/torus_single_roi_waypoint_sc_1_ -outputfile torus/results/torus_single_roi_waypoint_sc_max_1 -gzip
...
track -inputfile torus/torus_06.Bdouble.gz -inputmodel dt -seedfile torus/rois/torus_conmat_targets.nii.gz -anisthresh 0.25 | conmat -targetfile torus/rois/torus_conmat_targets.nii.gz -outputroot torus/results/torus_conmat_ -targetnamefile torus/rois/conmat_names.txt
