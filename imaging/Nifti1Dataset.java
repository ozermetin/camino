 ...
 * multi-valued data set) images. The fourth dimension of an image is time;
 * Camino doesn't support multiple time series in most programs so they are
 * likewise not supported in the Camino extensions to this class. The methods
 * that return DataSource objects deal with volumes of dimension 
 * [x][y][z][1][N], eg for diffusion tensors [x][y][z][1][6].
 *
 *
 *
 * @author Philip Cook
...
public final class Nifti1Dataset extends ImageHeader {
    ...
    protected void writeImage(double[][][][] data) {
        ...
        try {
            ...
            if (gzip) {
                hdrOut = new BufferedOutputStream(new GZIPOutputStream(hdrFos, BUFFERSIZE / 4), BUFFERSIZE);
            }
            else {
            ...
            if (ds_is_nii) {
                ...
                if (gzip) {
                    dsOut = new BufferedOutputStream(new GZIPOutputStream(dataFos, BUFFERSIZE / 4), BUFFERSIZE);
                }
                else {
    ...
    public RealMatrix getVoxelToPhysicalTransform() {
        ...
        else if (qform_code > 0 && qform_code < 3) {
	...
	if (!(dim[0] > 2 && dim[0] < 6)) {
	    throw new LoggedException("Can't handle this " + dim[0] + "-dimensional dataset. Camino " +
				      "supports 3D or 4D scalar volumes (where fourth dimension is 1) " + 
				      "or 5D multivariate datasets");
	}
	...
	data = new double[XDIM][YDIM][ZZZ][comps];
        ...
        try {
            ...
            if (ds_datname.endsWith(".gz")) {
                ...
                // buffer this much on the (compressed) disk side, twice as much on the uncompressed side
                int bufferSize = 1024*1024*8;
                
                is = new BufferedInflaterInputStream(new GZIPInputStream(new FileInputStream(ds_datname), bufferSize), bufferSize * 2);
            ...
            // could just call getImageDataSource(), which would save all this code, but costs more memory
            
            
            // Do the data type determination once, then loop inside. More code but faster
            switch (datatype) {
            ...
            case NIFTI_TYPE_INT8:
                for (n=0; n<comps; n++)
                    for (k=0; k<ZZZ; k++)
                        for (j=0; j<YDIM; j++)
                            for (i=0; i<XDIM; i++) {
                                data[i][j][k][n] = (double) (ecs.readByte());
                                if (scl_slope != 0.0)
                                    data[i][j][k][n] = data[i][j][k][n] * scl_slope + scl_inter;
                            }
                break;
            ...
            case NIFTI_TYPE_UINT8:
                for (n=0; n<comps; n++)
                    for (k=0; k<ZZZ; k++)
                        for (j=0; j<YDIM; j++)
                            for (i=0; i<XDIM; i++) {
                                data[i][j][k][n] = (double) (ecs.readUnsignedByte());
                                if (scl_slope != 0.0)
                                    data[i][j][k][n] = data[i][j][k][n] * scl_slope + scl_inter;
                            }
                break;
            ...
            case NIFTI_TYPE_INT16:
                for (n=0; n<comps; n++)
                    for (k=0; k<ZZZ; k++)
                        for (j=0; j<YDIM; j++)
                            for (i=0; i<XDIM; i++) {
                                data[i][j][k][n] = (double) (ecs.readShort());
                                if (scl_slope != 0.0)
                                    data[i][j][k][n] = data[i][j][k][n] * scl_slope + scl_inter;
                            }
                break;
            ...
            case NIFTI_TYPE_UINT16:
                for (n=0; n<comps; n++)
                    for (k=0; k<ZZZ; k++)
                        for (j=0; j<YDIM; j++)
                            for (i=0; i<XDIM; i++) {
                                data[i][j][k][n] = (double) (ecs.readUnsignedShort());
                                if (scl_slope != 0.0)
                                    data[i][j][k][n] = data[i][j][k][n] * scl_slope + scl_inter;
                            }
                break;
            ...
            case NIFTI_TYPE_INT32:
                for (n=0; n<comps; n++)
                    for (k=0; k<ZZZ; k++)
                        for (j=0; j<YDIM; j++)
                            for (i=0; i<XDIM; i++) {
                                data[i][j][k][n] = (double) (ecs.readInt());
                                if (scl_slope != 0.0)
                                    data[i][j][k][n] = data[i][j][k][n] * scl_slope + scl_inter;
                            }
                break;
            ...
            case NIFTI_TYPE_UINT32:
                for (n=0; n<comps; n++)
                    for (k=0; k<ZZZ; k++)
                        for (j=0; j<YDIM; j++)
                            for (i=0; i<XDIM; i++) {
                                data[i][j][k][n] = (double) (ecs.readUnsignedInt());
                                if (scl_slope != 0.0)
                                    data[i][j][k][n] = data[i][j][k][n] * scl_slope + scl_inter;
                            }
                break;
            ...
            case NIFTI_TYPE_INT64:
                for (n=0; n<comps; n++)
                    for (k=0; k<ZZZ; k++)
                        for (j=0; j<YDIM; j++)
                            for (i=0; i<XDIM; i++) {
                                data[i][j][k][n] = (double) (ecs.readLong());
                                if (scl_slope != 0.0)
                                    data[i][j][k][n] = data[i][j][k][n] * scl_slope + scl_inter;
                            }
                break;
            ...
            case NIFTI_TYPE_FLOAT32:
                for (n=0; n<comps; n++)
                    for (k=0; k<ZZZ; k++)
                        for (j=0; j<YDIM; j++)
                            for (i=0; i<XDIM; i++) {
                                data[i][j][k][n] = (double) (ecs.readFloat());
                                if (scl_slope != 0.0)
                                    data[i][j][k][n] = data[i][j][k][n] * scl_slope + scl_inter;
                            }
                break;
    ...
    public ImageHeader writeVectorImage(double[][][][] data, String fileRoot) {
        ...
        nds.dim[0] = 5;
        nds.dim[4] = 1;
        nds.dim[5] = (short)data[0][0][0].length;
        nds.dim[6] = 0;
        nds.dim[7] = 0;
	...
	DecimalFormat df = new DecimalFormat("0000");
        ...
        double[][][][] data = input.readVolumeData();

	for (int n = 0; n < numVols; n++) {

            double[][][][] vol = new double[input.XDIM][input.YDIM][input.ZDIM][1];

            for (int i = 0; i < input.XDIM; i++) {
                for (int j = 0; j < input.YDIM; j++) {
                    for (int k = 0; k < input.ZDIM; k++) { 
                        vol[i][j][k][0] = data[i][j][k][n];
                    }
                }
            }
