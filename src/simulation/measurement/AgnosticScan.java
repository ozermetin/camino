 ...
 * the scan. 
 * 
 * It can work with any SimulableScheme object
 *
 *
...
public class AgnosticScan implements SyntheticScan {
    ...
    /** 
     * maps a phase into the interval [0, 2pi). This was originally a helper method in {@link Walker}
     ...
     * 
     * @return rawPhase mod pi
     */
    public static final double mapToCircle(double rawPhase){
        
        double windingNum= Math.floor(rawPhase/(2.0*Math.PI));
    
        double retVal= Math.abs(rawPhase - windingNum*2.0*Math.PI);
        return retVal;
    }
    ...
    public double[] getSignalsFromTrajectories(){
    	...
    	// accumulate phase sifts for all walkers accros their trajectories
