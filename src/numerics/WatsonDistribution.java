...
/**
 * Watson Distribution, kind of like a Fisher but for axial data, ie p(x) ==
 * p(-x).
 * 
 * @see section 9.4.2 of Mardia and Jupp ("Directional Statistics", Wiley, 2000)
 * @version $Id: WatsonDistribution.java 58 2006-10-26 11:38:18Z ucacmgh $
 * @author Philip Cook
 */
public final class WatsonDistribution implements AxialDistribution {

    public final static boolean NEGATED = false;
    ...
    /**
     * @param mean
     *            the mean direction of a bipolar distribution (or polar
     *            direction of a girdle distribution).
     */
	...
	if (Math.abs(1.0 - mean.mod()) > 0.000001) {
	    throw new java.lang.IllegalArgumentException(
		    "Mean axis of Watson distribution must be a unit vector. Magnitude of mean was "
			    + mean.mod());
	}
     ...
     *            direction of a girdle distribution).
     * @param k
     *            the concentration that will be used if the no-arg nextVector()
     *            is called.
     */
    ...
    /**
     * Evaluate the pdf of the distribution. This integrates to 4.0 * PI over
     * the sphere.
     * 
     * @param x
     *            a unit vector on the unit sphere.
     */
    ...
    /**
     * Evaluate the pdf of any Watson distribution. This integrates to 4.0 * PI
     * over the sphere.
     * 
     * @param mean
     *            the mean axis of the distribution.
     * @param x
     *            a unit vector on the unit sphere.
     * @param k
     *            the concentration parameter of the distribution.
	...
	if (k < 700.0) {
	    ...
	    return Math.exp(k * muDotX * muDotX)
		    / hyper1F1(0.5, 1.5, k, 1.0e-9);
	} else {
	...
	// the extra method call slows the process of getting a vector by about
	// 10%.
	...
	if (k < 0.0) {
	    ...
	    // Negation if ON
	    if (NEGATED && ran.nextDouble() > 0.5) {
		// System.out.println("Negated");
		sample = sample.negated();
	    } else {
		// System.out.println("Not Negated");
	    }
	...
	// Now rotate sample about the mean axis by random angle gamma, between
	// 0 and 2 * PI
	...
	double sampX = (mu.x * mu.x * (1.0 - cg) + cg) * sample.x
		+ (mu.x * mu.y * (1.0 - cg) - mu.z * sg) * sample.y
		+ (mu.x * mu.z * (1.0 - cg) + mu.y * sg) * sample.z;

	double sampY = (mu.y * mu.x * (1.0 - cg) + mu.z * sg) * sample.x
		+ (mu.y * mu.y * (1.0 - cg) + cg) * sample.y
		+ (mu.y * mu.z * (1.0 - cg) - mu.x * sg) * sample.z;

	double sampZ = (mu.z * mu.x * (1.0 - cg) - mu.y * sg) * sample.x
     ...
     */
    public static Vector3D nextVector(double meanTheta, double meanPhi,
	    double k, Random ran) {
    ...
    /**
     * @return the next vector from the distribution. Approximates a computable
     *         solution for k large.
     */
    private static Vector3D nextVectorLargeK(double meanTheta, double meanPhi,
	    double k, Random ran) {
    ...
    /**
     * Computes the confluent hypergeometric function of the first kind,
     * M(a,b,x).
     * 
     * @param eps
     *            the maximum fractional error. The function will return when
     *            the series has converged to within this amount.
     * 
     *            This is taken, with some modification from Thompson, "Atlas
     *            for computing mathematical functions: an illustrated guide for
     *            practitioners, with programs in C and Mathematica" (Wiley,
     *            1997)
     * 
     */
	...
	if (x >= asympThresh) {
	    if (a <= 0.0) {
		System.err.println("a <= 0.0 and x >= " + asympThresh
			+ ". 0 returned");
		return 0.0;
	    }
		...
		if (termnew > termold) {
		    System.err
			    .println("Series diverging after " + k + " terms");
		    ratio = 0.0; // returns
		} else {
		    termold = termnew;
		}
	    ...
	    return Math.exp(GammaFunctions.gammln(b) + x
		    - GammaFunctions.gammln(a))
		    * Math.pow(x, a - b) * sum;
	...
	return Math
		.exp(GammaFunctions.gammln(b) - GammaFunctions.gammln(b - a))
		* sum / Math.pow(mx, a);
	...
	return (GammaFunctions.gammln(b) + x - GammaFunctions.gammln(a))
		+ Math.log(Math.pow(x, a - b)) + Math.log(sum);
