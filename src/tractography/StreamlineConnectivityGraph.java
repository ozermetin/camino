...
public class StreamlineConnectivityGraph extends ConnectivityMatrix {
	...
	int numPoints = t.numberOfPoints();
        ...
        int seedTargetIndex = targets[i][j][k];
        ...
        for (int p = tractSeedIndex; p < numPoints; p++) {
            ...
            if (voxelTarget > 0 && voxelTarget != seedTargetIndex) {
        ...
        if (seedTargetIndex > 0) {
            
            if (forwardTarget > 0 && backwardTarget > 0) {
                return; // connects three regions, ignore
            }
            else if (forwardTarget == 0) {
                forwardTarget = seedTargetIndex;
            }
            else {
                backwardTarget = seedTargetIndex;
            }
