...
public abstract class BayesDiracRandomizer {
	...
	for (int n = 0; n < numVectors; n++) {
	    ...
	    // we expand the likelihood of each term by 0.0005%
	    cdf[n] = previous + likelihood[i][j][k][n] * 1.000005f;
 	...
 	if (1.0 - r < 1E-6) {
	    // inefficiently search for ML vector but that's OK because this only happens
	    // less than once in a million samples
	    double ml = 0.0;
	    int mlInd = 0;
