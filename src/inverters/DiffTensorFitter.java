 ...
 * </dl>
 * 
 * @version $Id: DiffTensorFitter.java 1031 2012-02-20 21:01:13Z ucacpco $
...
public class DiffTensorFitter extends TensorModelFitter {
    ...
    public void setStartPoint(double[] dtParams) {
	    ...
	    // If L1 and L2 are positive, maybe we can recover a tensor that is somewhat sensible
            ...
            if (seig[0][1] > 0.0) {
              // set third eigenvalue to second eigenvalue
              seig[0][2] = seig[0][1];

              double traceD =  seig[0][0] + seig[0][1] + seig[0][2];

              modDT = DT.dtFromEig(seig);
              
            }
            else {
              // Usually this happens in anisotropic voxels, so md = L1 / 2 should 
	      // be in the right ballparck
              double md = Math.abs(seig[0][0]) / 2.0;
	      modDT = new DT(md, 0.0, 0.0, md, 0.0, md);
            }
  
            logger.info("Chol failure.  Negative evals in linear fitted DT.  Using modified DT for starting point. (traceD, fa) = (" + modDT.trace() + " , " + modDT.fa() + ")" );
