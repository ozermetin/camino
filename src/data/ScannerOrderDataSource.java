...
public class ScannerOrderDataSource extends ExternalDataSource {
    ...
    public ScannerOrderDataSource(String filename, int numVox, int components, String type, int[] dimensions) {
    	initFileInput(filename, false, 0);
            
    	init(numVox, components, type, dimensions);
     }


    private void init(int numVox, int components, String type, int[] dimensions) {
    ...
    /**
     * Coordinates starts from 0 to dimensions;
     * 
     * @param x
     * @param y
     * @param z
     * @return
     */
    public double[] getVoxelFromCoordinates(int x, int y, int z)
    {
    	int index = x+dimensions[0]*y+dimensions[1]*z;
    	return (data[index]);
    }
